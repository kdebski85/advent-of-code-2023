    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < stars.size(); i++) {
            ...
            for (int j = i + 1; j < stars.size(); j++) {
                ...
                result +=
                    Math.abs(distance.Δc()) +
                        Math.abs(distance.Δr()) +
                        (1000000L - 1L) * Math.abs(emptyRowsBefore[star1.y()] - emptyRowsBefore[star2.y()]) +
                        (1000000L - 1L) * Math.abs(emptyCollsBefore[star1.x()] - emptyCollsBefore[star2.x()]);
            }
        }
